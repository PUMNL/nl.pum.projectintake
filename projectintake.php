...
function projectintake_civicrm_aclWhereClause($type, &$tables, &$whereTables, &$contactID, &$where) {
  ...
  if ($customer_contact_rel_type_id === false) {
    return false;
  }
  ...
  while ($dao->fetch()) {
    ...
    if ($cc_rel_type_id !== false) {
      $tables[$cc_table_name] = $whereTables[$cc_table_name] = "LEFT JOIN `civicrm_relationship` `{$cc_table_name}` ON contact_a.id = {$cc_table_name}.contact_id_b";
    ...
    if ($loc_rep_rel_type_id !== false) {
      $tables[$loc_rep_table_name] = $whereTables[$loc_rep_table_name] = "LEFT JOIN `civicrm_relationship` `{$loc_rep_table_name}` ON contact_a.id = {$loc_rep_table_name}.contact_id_b";
